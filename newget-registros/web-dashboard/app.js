...
function criarCardLoja(codigo, loja) {
    ...
    // Se a loja estiver offline, exibir "indefinido" para o status do totem
    if (statusInfo.status === "Offline") {
        motherboardStatusText.textContent = "indefinido";
...
function criarLinhaTabelaLoja(codigo, loja) {
    ...
    if (statusInfo.status === "Offline") {
        if (motherboardText) {
            motherboardText.textContent = "indefinido";
            motherboardText.className = "text-muted fw-medium motherboard-status";
        }
        if (motherboardIndicator) {
            motherboardIndicator.className = "motherboard-indicator me-2";
        }
    } else if (motherboardStatus) {
        const isMotherboardOn = motherboardStatus === 'ON';
        if (motherboardText) {
            motherboardText.textContent = motherboardStatus;
            motherboardText.className = `${isMotherboardOn ? 'text-success' : 'text-danger'} fw-medium motherboard-status`;
        }
    ...
    if (btnAcessar) {
        btnAcessar.href = `loja.html?id=${codigo}`;
        // Atualiza o botão conforme o status
        if (statusInfo.status === 'Offline') {
            btnAcessar.classList.add('disabled');
            ...
            btnAcessar.removeAttribute('title');
            btnAcessar.classList.remove('btn-outline-secondary');
            btnAcessar.classList.add('btn-outline-primary');
            ...
            lojasTableBody.appendChild(fragmento);
                ...
                try {
                    lojasTable = $('#lojas-table').DataTable({
                        ...
                        columnDefs: [
                            { orderable: false, targets: 5 }, // Desabilitar ordenação para a coluna de ações
                            { className: "text-start", targets: 4 } // Alinhar à esquerda a coluna de última atualização
                        ],
                        responsive: true,
