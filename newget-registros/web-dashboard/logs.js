...
// Normaliza o tipo de operação para exibição
function normalizarTipoOperacao(tipo) {
    if (!tipo) return 'Desconhecido';
    
    // Converter para string e em minúsculas para facilitar comparação
    const tipoLowerCase = String(tipo).toLowerCase();
    
    // Casos especiais
    if (tipoLowerCase.includes('secadora') || tipoLowerCase.includes('liberacao_secadora')) {
        return 'Secadora';
    } else if (tipoLowerCase.includes('lavadora') || tipoLowerCase.includes('liberacao_lavadora')) {
        return 'Lavadora';
    } else if (tipoLowerCase.includes('dosadora') || tipoLowerCase.includes('acionamento_dosadora')) {
        return 'Dosadora';
    } else if (tipoLowerCase.includes('reset')) {
        return 'Reset';
    } else if (tipoLowerCase.includes('configuracao') || tipoLowerCase.includes('config')) {
        return 'Configuração';
    } else if (tipoLowerCase.includes('ar_condicionado') || tipoLowerCase.includes('arcondicionado')) {
        return 'Ar-Condicionado';
    } else if (tipoLowerCase.includes('batch') || tipoLowerCase.includes('lote')) {
...
function buscarRegistrosDoFirebase(atualizacaoEmSegundoPlano = false) {
    ...
    // Debug para operações de secadora
    console.log("[DEBUG] Verificando operações de secadora especificamente...");
    firebase.firestore().collection(colecaoRegistros)
        .where("tipoOperacao", "==", "liberacao_secadora")
        .orderBy("timestamp", "desc")
        .limit(5)
        .get()
        .then(snapshot => {
            console.log(`[DEBUG] Encontradas ${snapshot.size} operações de secadora no Firestore`);
            snapshot.forEach(doc => {
                const data = doc.data();
                console.log(`[DEBUG] Secadora ${data.secadoraId}: status=${data.configuracao?.status || '?'}, tempo=${data.configuracao?.tempo || '?'} min`);
            });
        })
        .catch(error => {
            console.error("[DEBUG] Erro ao verificar operações de secadora:", error);
        });
    
    // Debug para operações de ar-condicionado
    console.log("[DEBUG] Verificando operações de ar-condicionado especificamente...");
    firebase.firestore().collection(colecaoRegistros)
        .where("tipoOperacao", "==", "operacao_ar_condicionado")
        .orderBy("timestamp", "desc")
        .limit(5)
        .get()
        .then(snapshot => {
            console.log(`[DEBUG] Encontradas ${snapshot.size} operações de ar-condicionado no Firestore`);
            snapshot.forEach(doc => {
                console.log(`[DEBUG] Operação ar-condicionado:`, doc.id, doc.data());
            });
        })
        .catch(error => {
            console.error("[DEBUG] Erro ao buscar operações de ar-condicionado:", error);
        });
...
function carregarLogsDoCache() {
    try {
        ...
        // Atualizar a indicação da fonte de dados com o tempo desde a última atualização
...
function processarRegistros(registros) {
    ...
    const registrosProcessados = registrosUnicos.map(registro => {
        ...
        // Campo para dispositivo/máquina
        if (registro.tipoOperacao && registro.tipoOperacao.toLowerCase().includes('ar_condicionado')) {
            // Para ar condicionado, mostramos o dispositivo como "AC"
            registro.dispositivo = "AC";
        } else if (registro.configuracao && registro.configuracao.dispositivo) {
            registro.dispositivo = registro.configuracao.dispositivo.replace('secadora_', '').replace('lavadora_', '').replace('dosadora_', '');
        }
        
        // Verifica se é uma operação de ar-condicionado
        if (registro.tipoOperacao && registro.tipoOperacao.toLowerCase().includes('ar_condicionado')) {
            // Retorna a temperatura configurada
            console.log("[DEBUG] Processando registro de ar-condicionado:", registro);
            
            if (registro.configuracao && registro.configuracao.temperatura !== undefined) {
                const temp = registro.configuracao.temperatura;
                console.log(`[DEBUG] Temperatura encontrada: ${temp}`);
                return temp === 'OFF' ? 'Desligado' : `${temp}°C`;
            }
            
            // Tentativas alternativas de encontrar o valor da temperatura
            if (registro.temperatura !== undefined) {
                return registro.temperatura === 'OFF' ? 'Desligado' : `${registro.temperatura}°C`;
            }
            
            if (registro.dados && registro.dados.temperatura !== undefined) {
                return registro.dados.temperatura === 'OFF' ? 'Desligado' : `${registro.dados.temperatura}°C`;
            }
        }
...
function renderizarRegistros(registros) {
    ...
    registros.forEach(registro => {
        ...
        // Verificar se o registro tem dados suficientes para ser exibido
        // Só adicionar à tabela se tiver pelo menos data/hora e loja ou tipo de operação
        if ((dataFormatada !== 'N/A' || horarioFormatado !== 'N/A') && 
            (loja || operacao)) {
            // Adicionar linha com dados formatados
            dataTable.row.add([
                dataFormatada,
                horarioFormatado,
                loja || 'N/A',
                usuario || 'N/A',
                operacao ? gerarBadgeOperacao(operacao) : 'N/A',
                maquina || 'N/A',
                produto || 'N/A',
                dosagem || 'N/A',
                tempo || 'N/A'
            ...
            console.log('Registro ignorado por falta de dados suficientes:', registro);
        }
    });
...
function obterProduto(registro) {
    // Verifica se é uma operação de ar-condicionado
    if (registro.tipoOperacao && registro.tipoOperacao.toLowerCase().includes('ar_condicionado') || 
        (registro.operacao && registro.operacao === 'Ar-Condicionado')) {
        
        // Começa com um log para depuração
        console.log("[DEBUG] Processando registro de ar-condicionado:", registro);
        
        // Tenta encontrar a temperatura na seguinte ordem de prioridade:
        
        // 1. Verifica se o campo temperatura existe diretamente no registro
        if (registro.temperatura !== undefined) {
            const temp = registro.temperatura;
            console.log(`[DEBUG] Temperatura encontrada diretamente: ${temp}`);
            return temp === 'OFF' ? 'Desligado' : `${temp}°C`;
        }
        
        // 2. Verifica se existe dentro de configuracao
        if (registro.configuracao && registro.configuracao.temperatura !== undefined) {
            const temp = registro.configuracao.temperatura;
            console.log(`[DEBUG] Temperatura encontrada em configuracao: ${temp}`);
            return temp === 'OFF' ? 'Desligado' : `${temp}°C`;
        }
        
        // 3. Verifica em dados
        if (registro.dados && registro.dados.temperatura !== undefined) {
            const temp = registro.dados.temperatura;
            console.log(`[DEBUG] Temperatura encontrada em dados: ${temp}`);
            return temp === 'OFF' ? 'Desligado' : `${temp}°C`;
        }
        
        // 4. Se não encontrar em nenhum lugar, retorna um valor padrão
        return 'Temperatura não definida';
    }
