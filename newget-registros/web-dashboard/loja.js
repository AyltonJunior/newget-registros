...
function setupLavadoraControls(card, id) {
    ...
    // Função para atualizar o estado dos controles
    function updateControls(dosadoraStatus, lavadoraStatus) {
        const isEnabled = dosadoraStatus === 'online' && lavadoraStatus === 'online';
        
        if (btnAplicar) {
            btnAplicar.disabled = !isEnabled;
            btnAplicar.title = !isEnabled ? 
                (lavadoraStatus !== 'online' ? 'A lavadora está offline. Não é possível liberar.' : 'A dosadora está offline. Não é possível liberar.') : 
                '';
        }
        if (amacianteSelect) amacianteSelect.disabled = !isEnabled;
        if (dosagemSelect) dosagemSelect.disabled = !isEnabled;
    }

    // Listener para status da dosadora
    const dosadoraStatusRef = database.ref(`/${lojaId}/status/dosadoras/${id}`);
    dosadoraStatusRef.on('value', (dosadoraSnapshot) => {
        const dosadoraStatus = dosadoraSnapshot.val();
        
        // Buscar status atual da lavadora
        const lavadoraStatusRef = database.ref(`/${lojaId}/status/lavadoras/${id}`);
        lavadoraStatusRef.once('value').then(lavadoraSnapshot => {
            const lavadoraStatus = lavadoraSnapshot.val();
            updateControls(dosadoraStatus, lavadoraStatus);
        });
    });

    // Listener para status da lavadora
    const lavadoraStatusRef = database.ref(`/${lojaId}/status/lavadoras/${id}`);
    lavadoraStatusRef.on('value', (lavadoraSnapshot) => {
        const lavadoraStatus = lavadoraSnapshot.val();
        
        // Buscar status atual da dosadora
        const dosadoraStatusRef = database.ref(`/${lojaId}/status/dosadoras/${id}`);
        dosadoraStatusRef.once('value').then(dosadoraSnapshot => {
            const dosadoraStatus = dosadoraSnapshot.val();
            updateControls(dosadoraStatus, lavadoraStatus);
        });
    });

    // Handler do botão liberar
    if (btnAplicar) {
        btnAplicar.addEventListener('click', () => {
            ...
            lavadoraRef.update({
                ...
                const dosadoraStatusListener = dosadoraStatusRef.on('value', (snapshot) => {
                    ...
                    const amacianteSelect = cardElement.querySelector('.amaciante-select');
                    const dosagemSelect = cardElement.querySelector('.dosagem-select');
                    ...
                    if (status === 'online') {
                        ...
                        if (amacianteSelect) amacianteSelect.disabled = false;
                        if (dosagemSelect) dosagemSelect.disabled = false;
                        ...
                        if (amacianteSelect) amacianteSelect.disabled = true;
                        if (dosagemSelect) dosagemSelect.disabled = true;
...
function criarCardLavadora(id, dados) {
    ...
    if (dados.liberando === true) {
        ...
        // Inicialmente esconde o badge até receber o status do Firebase
        statusBadge.style.display = '';
        statusIndicator.style.display = '';
    }
    
    // Configura listener para o status da lavadora em tempo real
    const lavadoraStatusRef = database.ref(`/${lojaId}/status/lavadoras/${id}`);
    lavadoraStatusRef.on('value', (snapshot) => {
        const status = snapshot.val();
        if (status === 'online') {
    ...
    // Configura listener para o status_machine em tempo real (disponibilidade)
    const statusMachineRef = database.ref(`/status_machine/${lojaId}/${id}/status`);
    statusMachineRef.on('value', (snapshot) => {
        ...
        if (availability === 'available') {
            ...
            statusBadge.className = 'badge bg-warning device-status text-dark';
        } else if (availability === 'suspended') {
    ...
    // Desabilita os dropdowns se a dosadora não estiver online
    if (dados.dosadoraStatus !== 'online') {
        if (amacianteSelect) amacianteSelect.disabled = true;
        if (dosagemSelect) dosagemSelect.disabled = true;
    } else {
    ...
    // Configura os controles da lavadora
    setupLavadoraControls(card, id);
...
function criarCardSecadora(id, dados) {
    ...
    // Remover badge de status e container de status
    const statusBadge = card.querySelector('.device-status');
    if (statusBadge) {
        statusBadge.remove();
    }
    const secadoraStatusContainer = card.querySelector('.secadora-status-container');
    if (secadoraStatusContainer) {
        secadoraStatusContainer.remove();
    }
    ...
    secadoraRef.on('value', (snapshot) => {
        ...
        if (statusAtivo) {
            btnIniciar.disabled = true;
            btnIniciar.innerHTML = '<i class="fas fa-check-circle me-2"></i>Em andamento';
    ...
    btnIniciar.addEventListener('click', () => {
        ...
        let timeoutId = setTimeout(() => {
            showAlert('A liberação da secadora demorou mais de 5 segundos. Verifique a conexão e tente novamente.', 'Erro', 'error');
            btnIniciar.disabled = false;
            btnIniciar.innerHTML = '<i class="fas fa-play-circle me-2"></i>Liberar';
            tempoSelect.disabled = false;
            // Define o status da secadora como offline no Firebase
            const statusRef = database.ref(`/${lojaId}/status/secadoras/${id}`);
            statusRef.set('offline');
            // Atualiza o status visual do card imediatamente
            const cardElement = btnIniciar.closest('.card');
            if (cardElement) {
        ...
        secadoraRef.once('value')
            ...
            .then(() => {
                // Configura um listener para monitorar o status da secadora
                const statusSecadoraRef = database.ref(`/${lojaId}/status/secadoras/${id}`);
                const statusListener = statusSecadoraRef.on('value', (snapshot) => {
                    const statusSecadora = snapshot.val();
                    if (statusSecadora === 'online' || statusSecadora === 'offline') {
                        clearTimeout(timeoutId);
                    }
                    // ... restante do listener ...
                });
                // ... restante do código ...
            })
                        .catch(error => {
    ...
    // Adicionar status visual e bolinha igual lavadora
    const deviceHeader = card.querySelector('.device-header');
    if (deviceHeader) {
        // Cria a bolinha de status
        const statusIndicator = document.createElement('span');
        statusIndicator.className = 'status-indicator';
        // Cria o badge de status
        const statusBadge = document.createElement('span');
        statusBadge.className = 'badge device-status';
        // Define status inicial
        if (dados.secadoraStatus === 'online') {
            statusIndicator.classList.add('status-online');
            statusBadge.textContent = 'Online';
            statusBadge.classList.add('bg-success');
        } else if (dados.secadoraStatus === 'offline') {
            statusIndicator.classList.add('status-offline');
                        statusBadge.textContent = 'Offline';
            statusBadge.classList.add('bg-danger');
        } else if (dados.secadoraStatus === 'liberando') {
        ...
        h5.className = 'mb-0';
        h5.appendChild(statusIndicator);
        h5.appendChild(document.createTextNode(' Secadora '));
        const deviceIdSpan = document.createElement('span');
        deviceIdSpan.className = 'device-id';
        deviceIdSpan.textContent = id;
        h5.appendChild(deviceIdSpan);
        deviceHeader.appendChild(h5);
        deviceHeader.appendChild(statusBadge);
    }
    
    // Após criar o card e adicionar ao DOM, adicione um listener em tempo real para o status da secadora
    const statusRef = database.ref(`/${lojaId}/status/secadoras/${id}`);
    statusRef.on('value', (snapshot) => {
        const status = snapshot.val();
        const cardElement = secadorasContainer.querySelector(`.card-secadora-${id}`);
        if (cardElement) {
            const statusIndicator = cardElement.querySelector('.status-indicator');
            const statusBadge = cardElement.querySelector('.device-status');
            const btnIniciar = cardElement.querySelector('.btn-iniciar');
            if (status === 'online') {
    ...
    // Após criar o card, desabilite o botão se status não for online
    const btnIniciarInit = card.querySelector('.btn-iniciar');
    if (dados.secadoraStatus !== 'online') {
        if (btnIniciarInit) btnIniciarInit.disabled = true;
    } else {
        if (btnIniciarInit) btnIniciarInit.disabled = false;
    }
...
function criarCardDosadora(id, dados) {
    ...
    statusRef.on('value', (snapshot) => {
        ...
        const status = snapshot.val();
        const cardElement = cardDiv; // cardDiv já é a referência ao card
        const btnAcionar = cardElement.querySelector('.btn-acionar');
        const btnConfigurar = cardElement.querySelector('.btn-configurar');
        const btnConsultar = cardElement.querySelector('.btn-consultar');
        const bombaSelect = cardElement.querySelector('.bomba-select');
        
        if (status === 'online') {
            statusBadge.textContent = 'Online';
            statusBadge.className = 'badge bg-success device-status';
            statusIndicator.className = 'status-indicator status-online';
            if (btnAcionar) btnAcionar.disabled = false;
            if (btnConfigurar) btnConfigurar.disabled = false;
            if (btnConsultar) btnConsultar.disabled = false;
            if (bombaSelect) bombaSelect.disabled = false;
        } else if (status === 'offline') {
            statusBadge.textContent = 'Offline';
            statusBadge.className = 'badge bg-danger device-status';
            statusIndicator.className = 'status-indicator status-offline';
            if (btnAcionar) btnAcionar.disabled = true;
            if (btnConfigurar) btnConfigurar.disabled = true;
            if (btnConsultar) btnConsultar.disabled = true;
            if (bombaSelect) bombaSelect.disabled = true;
    ...
    btnAcionar.addEventListener('click', () => {
        // Verifica status da dosadora antes de acionar (garantia extra)
        const statusRef = database.ref(`/${lojaId}/status/dosadoras/${id}`);
        statusRef.once('value').then(snapshot => {
            const status = snapshot.val();
            if (status !== 'online') {
                showAlert('A dosadora está offline ou não está disponível na rede. Não é possível acionar a bomba.', 'Erro', 'error');
        ...
        const dosadoraRef = database.ref(`/${lojaId}/dosadora_01/${id}`);
            dosadoraRef.update({ bomba: bomba }).then(() => {
                let timeoutId = setTimeout(() => {
                    showAlert('A dosadora não respondeu ao comando em 10 segundos. Verifique a conexão e tente novamente.', 'Erro', 'error');
    ...
    // Após criar o card, desabilite todos os botões se status não for online
    const btnAcionarInit = card.querySelector('.btn-acionar');
    const btnConfigurarInit = card.querySelector('.btn-configurar');
    const btnConsultarInit = card.querySelector('.btn-consultar');
    const allBtnsInit = [btnAcionarInit, btnConfigurarInit, btnConsultarInit];
    if (dados.status !== 'online') {
        allBtnsInit.forEach(btn => { if (btn) btn.disabled = true; });
    } else {
        allBtnsInit.forEach(btn => { if (btn) btn.disabled = false; });
        }
...
function criarCardAr(dados) {
    ...
    let tempAtiva = null;
    if (dados) {
        console.log("Dados do ar-condicionado ao criar card:", JSON.stringify(dados));
        
        // Adiciona verificação de tipo para garantir que estamos comparando valores booleanos
        // Verifica cada possível valor (18, 22, OFF)
        if (dados["18"] === true || dados["18"] === "true") {
            tempAtiva = "18";
            console.log("Temperatura 18°C ativa no card (valor:", dados["18"], ", tipo:", typeof dados["18"], ")");
        } else if (dados["22"] === true || dados["22"] === "true") {
            tempAtiva = "22";
            console.log("Temperatura 22°C ativa no card (valor:", dados["22"], ", tipo:", typeof dados["22"], ")");
        } else if (dados["OFF"] === true || dados["OFF"] === "true") {
            tempAtiva = "OFF";
            console.log("Ar condicionado OFF no card (valor:", dados["OFF"], ", tipo:", typeof dados["OFF"], ")");
        } else if (dados.temp) {
            // Compatibilidade com formato antigo
            tempAtiva = dados.temp;
            console.log("Formato antigo de temperatura detectado no card:", dados.temp);
        } else {
            console.log("Nenhuma temperatura ativa detectada no card");
        }
    }
    ...
    if (btnAplicarTemp) {
        btnAplicarTemp.addEventListener('click', () => {
            ...
            // Registra a operação no Firestore ao iniciar o processo
            registrarArCondicionado(lojaId, temperatura, 'iniciado')
            .then(() => {
                console.log(`[DEBUG] Registro inicial de ar-condicionado com temperatura=${temperatura} salvo com sucesso`);
            })
            .catch(error => {
                console.error(`Erro ao registrar operação de ar-condicionado no Firestore: ${error.message}`);
            });
            ...
            noRef.once('value')
                .then(snapshot => {
                    if (snapshot.exists() || ["18", "22", "OFF"].includes(temperatura)) {
                        // Em vez de usar updates com um objeto, vamos setar diretamente cada valor
                        // Desativar todas as temperaturas primeiro
                        const promises = [
                            database.ref(`/${lojaId}/ar_condicionado/18`).set(false),
                            database.ref(`/${lojaId}/ar_condicionado/22`).set(false),
                            database.ref(`/${lojaId}/ar_condicionado/OFF`).set(false)
                        ];
                        
                        // Aguardar a desativação completa antes de ativar a nova temperatura
                        return Promise.all(promises)
                            .then(() => {
                                console.log(`Todas as temperaturas desativadas, ativando ${temperatura}`);
                                // Agora ativamos apenas a temperatura desejada
                                return database.ref(`/${lojaId}/ar_condicionado/${temperatura}`).set(true);
                            });
                    } else {
                ...
                .then(() => {
                    ...
                    const statusListener = statusRef.on('value', (snapshot) => {
                        ...
                        if (status === 'online') {
                            ...
                            const tempText = temperatura === 'OFF' ? 'Desligado' : `${temperatura}°C`;
                            showAlert(`Temperatura do ar-condicionado configurada para ${tempText}`, 'Sucesso', 'success', true);
                            ...
                            // Registra a operação no Firestore como sucesso (apenas uma vez)
                            registrarArCondicionado(lojaId, temperatura, 'sucesso')
                            .then(() => {
                                console.log(`Operação de ar-condicionado registrada com sucesso: ${temperatura}`);
                            })
                            .catch(error => {
                                console.error(`Erro ao registrar sucesso de operação de ar-condicionado no Firestore: ${error.message}`);
                            });
                        ...
                        // Se o ESP atualizar o status para 'offline', significa que não foi possível completar a operação
                            ...
                            // Registra falha no Firestore
                            registrarArCondicionado(lojaId, temperatura, 'falha')
                            .catch(error => {
                                console.error(`Erro ao registrar falha de operação de ar-condicionado: ${error.message}`);
                            });
                            
                            // Mostra alerta de erro
                            showAlert('Não foi possível configurar o ar-condicionado. Verifique a conexão.', 'Erro', 'error', true);
...
function carregarAr() {
    ...
    .then(([arSnapshot, statusSnapshot]) => {
        ...
        statusRef.on('value', snapshot => {
            ...
            const statusIndicator = cardElement.querySelector('.status-indicator');
            const statusBadge = cardElement.querySelector('.device-status');
            const btnAplicarTemp = cardElement.querySelector('.btn-aplicar-temp');
            ...
            if (status === 'online') {
                ...
                if (btnAplicarTemp) btnAplicarTemp.disabled = false;
                if (temperaturaSelect) temperaturaSelect.disabled = false;
            } else if (status === 'offline') {
                if (statusBadge) {
                    statusBadge.textContent = 'Offline';
                    statusBadge.className = 'badge bg-danger device-status';
                ...
                if (btnAplicarTemp) btnAplicarTemp.disabled = true;
                if (temperaturaSelect) temperaturaSelect.disabled = true;
            } else if (status === 'processando' || status === 'liberando') {
                if (statusBadge) {
                    statusBadge.textContent = status === 'processando' ? 'Processando...' : 'Liberando...';
                    statusBadge.className = 'badge bg-info device-status';
                }
                if (statusIndicator) {
                    statusIndicator.className = 'status-indicator status-warning';
        ...
        arRef.on('value', snapshot => {
            ...
            let tempAtiva = null;
            console.log("Valores do ar_condicionado recebidos:", JSON.stringify(dados));

            // Usa comparação estrita === true ou compara com Boolean(true)
            if (dados["18"] === true) {
                tempAtiva = "18";
                console.log("Temperatura 18°C ativa");
            } else if (dados["22"] === true) {
                tempAtiva = "22";
                console.log("Temperatura 22°C ativa");
            } else if (dados["OFF"] === true) {
                tempAtiva = "OFF";
                console.log("Ar condicionado desligado (OFF)");
            } else if (dados.temp) {
                // Compatibilidade com formato antigo
                tempAtiva = dados.temp;
                console.log("Formato antigo de temperatura detectado:", dados.temp);
            } else {
                console.log("Nenhuma temperatura ativa detectada");
            }
...
function atualizarStatusMachine(tipo, id, status) {
    ...
    if (machineIndex === -1) {
        ...
        if (currentStatus !== status) {
            ...
            // Criar objeto de histrico com o evento de alterao
            const historyEntry = {
                timestamp: Date.now(),
                tipo: tipo,
                id: id,
            ...
            // Salvar a entrada de histrico no Firebase
...
// Função para iniciar uma secadora
function iniciarSecadora(id, tempo) {
    console.log(`Iniciando secadora ${id} com tempo ${tempo.replace('_', '')} minutos...`);
    console.log(`[DEBUG] Path a ser usado: /${lojaId}/secadoras/${id}${tempo}`);
    
    // Referência para este nó específico no Firebase
    const secadoraRef = database.ref(`/${lojaId}/secadoras/${id}${tempo}`);
    
    // Verificar se o nó existe antes de tentar definir como true
    secadoraRef.once('value')
        .then(snapshot => {
            if (snapshot.exists()) {
                console.log(`[DEBUG] Nó secadoras/${id}${tempo} encontrado, definindo como true`);
                return secadoraRef.set(true);
            } else {
                console.warn(`Nó secadoras/${id}${tempo} não encontrado no Firebase.`);
                throw new Error(`Secadora ${id} com tempo ${tempo.replace('_', '')} minutos não está configurada no sistema.`);
        ...
        .then(() => {
            console.log(`Secadora ${id} status atualizado para 'liberando'`);
